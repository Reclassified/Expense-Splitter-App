...
router.get('/:expenseId', authenticateToken, (req, res) => {
  try {
    ...
    // Check if user is member of the group
    const memberCheck = db.prepare('SELECT * FROM group_members WHERE group_id = ? AND user_id = ?').get(expense.group_id, userId);
    if (!memberCheck) {
      return res.status(403).json({ error: 'Access denied' });
    }
...
router.post('/', authenticateToken, (req, res) => {
  try {
    ...
    // Validation
    if (!groupId || !title || !amount || !memberIds || !Array.isArray(memberIds)) {
    ...
    const groupMembers = db.prepare('SELECT user_id FROM group_members WHERE group_id = ?').all(groupId);
    ...
    if (splits && Array.isArray(splits)) {
      ...
      for (const split of splits) {
        if (!memberIds.includes(split.userId)) {
          return res.status(400).json({ error: 'Split includes invalid member' });
        }
    ...
    const transaction = db.transaction(() => {
      // Insert expense
      const insertExpense = db.prepare('INSERT INTO expenses (group_id, title, amount, paid_by, description) VALUES (?, ?, ?, ?, ?)');
      const result = insertExpense.run(groupId, title.trim(), amount, userId, description?.trim() || null);
      
      const expenseId = result.lastInsertRowid;
      
      // Insert expense members
      const insertMember = db.prepare('INSERT INTO expense_members (expense_id, user_id, share_amount) VALUES (?, ?, ?)');
      for (const memberId of memberIds) {
        let shareAmount = splits && Array.isArray(splits) ? splitMap[memberId] : amount / memberIds.length;
    ...
    `).all(groupId);
    const balances = calculateBalances(groupMembers, expenses, expenseMembers);
...
router.put('/:expenseId', authenticateToken, (req, res) => {
  try {
    ...
    const canEdit = expense.paid_by === userId || memberCheck.role === 'owner' || memberCheck.role === 'admin';
    if (!canEdit) {
    ...
    // Check if all memberIds are valid group members
    const groupMembers = db.prepare('SELECT user_id FROM group_members WHERE group_id = ?').all(expense.group_id);
    const groupMemberIds = groupMembers.map(m => m.user_id);
    ...
    const transaction = db.transaction(() => {
      // Update expense
      const updateExpense = db.prepare('UPDATE expenses SET title = ?, amount = ?, description = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?');
      updateExpense.run(title.trim(), amount, description?.trim() || null, expenseId);
      
      // Remove existing members
      const deleteMembers = db.prepare('DELETE FROM expense_members WHERE expense_id = ?');
...
router.delete('/:expenseId', authenticateToken, (req, res) => {
  try {
    ...
    const canDelete = expense.paid_by === userId || memberCheck.role === 'owner';
    if (!canDelete) {
...
router.post('/recurring', authenticateToken, (req, res) => {
  try {
    const { groupId, title, amount, currency, paidBy, description, frequency, nextDueDate, endDate } = req.body;
    const userId = req.user.userId;
    if (!groupId || !title || !amount || !currency || !paidBy || !frequency || !nextDueDate) {
      return res.status(400).json({ error: 'Missing required fields' });
    }
    // Check if user is member of the group
    const memberCheck = db.prepare('SELECT * FROM group_members WHERE group_id = ? AND user_id = ?').get(groupId, userId);
    ...
    if (!(memberCheck.role === 'owner' || memberCheck.role === 'admin')) {
      return res.status(403).json({ error: 'Only owner or admin can create recurring expenses' });
    }
    const insert = db.prepare(`
      INSERT INTO recurring_expenses (group_id, title, amount, currency, paid_by, description, frequency, next_due_date, end_date)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    const result = insert.run(groupId, title.trim(), amount, currency, paidBy, description?.trim() || null, frequency, nextDueDate, endDate || null);
...
router.get('/recurring', authenticateToken, (req, res) => {
  try {
    ...
    const recurs = db.prepare('SELECT * FROM recurring_expenses WHERE group_id = ? AND is_active = 1').all(groupId);
    res.json(recurs);
  } catch (error) {
...
router.get('/recurring/:id', authenticateToken, (req, res) => {
  try {
    const { id } = req.params;
    const userId = req.user.userId;
    const recur = db.prepare('SELECT * FROM recurring_expenses WHERE id = ?').get(id);
    if (!recur) return res.status(404).json({ error: 'Not found' });
    // Check if user is member of the group
    const memberCheck = db.prepare('SELECT * FROM group_members WHERE group_id = ? AND user_id = ?').get(recur.group_id, userId);
    if (!memberCheck) return res.status(403).json({ error: 'Access denied' });
    res.json(recur);
  } catch (error) {
...
router.put('/recurring/:id', authenticateToken, (req, res) => {
  try {
    const { id } = req.params;
    const { title, amount, currency, paidBy, description, frequency, nextDueDate, endDate, isActive } = req.body;
    const userId = req.user.userId;
    const recur = db.prepare('SELECT * FROM recurring_expenses WHERE id = ?').get(id);
    if (!recur) return res.status(404).json({ error: 'Not found' });
    // Check if user is member of the group
    ...
    if (!(memberCheck.role === 'owner' || memberCheck.role === 'admin')) {
      return res.status(403).json({ error: 'Only owner or admin can update recurring expenses' });
    }
    ...
    `);
    update.run(title, amount, currency, paidBy, description, frequency, nextDueDate, endDate, isActive ? 1 : 0, id);
...
router.delete('/recurring/:id', authenticateToken, (req, res) => {
  try {
    ...
    if (!(memberCheck.role === 'owner' || memberCheck.role === 'admin')) {
      return res.status(403).json({ error: 'Only owner or admin can delete recurring expenses' });
    }
...
router.post('/group/:groupId/import/csv', authenticateToken, upload.single('file'), async (req, res) => {
    const { groupId } = req.params;
    const userId = req.user.userId;

    // Check if user is a member of the group and has permission (e.g., admin/owner)
    const memberCheck = db.prepare('SELECT role FROM group_members WHERE group_id = ? AND user_id = ?').get(groupId, userId);
    if (!memberCheck || (memberCheck.role !== 'owner' && memberCheck.role !== 'admin')) {
        return res.status(403).json({ error: 'You do not have permission to import expenses.' });
    }
    ...
    try {
        const groupMembers = db.prepare('SELECT user_id, username FROM group_members WHERE group_id = ?').all(groupId);
        ...
        const transaction = db.transaction(() => {
            for (const [index, expense] of expensesToCreate.entries()) {
                ...
                if (!paidByUserId) {
                    errors.push(`Row ${index + 2}: Payer "${paid_by}" is not in this group.`);
                    continue;
                }

                const memberUsernames = members.split(',').map(m => m.trim());
                const memberUserIds = memberUsernames.map(username => groupMemberUsernames[username]);

                if (memberUserIds.some(id => id === undefined)) {
                    errors.push(`Row ${index + 2}: One or more members in "${members}" are not in this group.`);
                    continue;
                }

                // Create expense
                const insertExpense = db.prepare('INSERT INTO expenses (group_id, title, amount, paid_by, description) VALUES (?, ?, ?, ?, ?)');
                const expenseResult = insertExpense.run(groupId, title, amount, paidByUserId, expense.description || '');
...
router.get('/group/:groupId/export/pdf', authenticateToken, (req, res) => {
    ...
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename=expenses-${groupId}.pdf`);
        doc.pipe(res);

        // Add content to the PDF
        doc.fontSize(20).text(`Expense Report for ${group.name}`, { align: 'center' });
        doc.moveDown();

        expenses.forEach(expense => {
            doc.fontSize(14).text(expense.title, { continued: true });
            doc.fontSize(14).text(` - ${expense.amount} ${expense.currency}`, { align: 'right' });
            doc.fontSize(10).text(`Paid by ${expense.paid_by} on ${new Date(expense.created_at).toLocaleDateString()}`);
