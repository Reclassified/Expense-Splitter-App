...
cron.schedule('0 * * * *', () => { // Every hour
  try {
    const now = new Date();
    const dueRecurs = db.prepare(`SELECT * FROM recurring_expenses WHERE is_active = 1 AND next_due_date <= ?`).all(now.toISOString());
    dueRecurs.forEach(recur => {
      // Insert new expense
      const insertExpense = db.prepare('INSERT INTO expenses (group_id, title, amount, paid_by, description, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)');
      const nowStr = new Date().toISOString();
      insertExpense.run(recur.group_id, recur.title, recur.amount, recur.paid_by, recur.description, nowStr, nowStr);
      ...
      else if (recur.frequency === 'monthly') next.setMonth(next.getMonth() + 1);
      else return; // Unknown frequency
      // Update next_due_date
      db.prepare('UPDATE recurring_expenses SET next_due_date = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').run(next.toISOString(), recur.id);
      // Optionally, deactivate if end_date is reached
      if (recur.end_date && new Date(next) > new Date(recur.end_date)) {
        db.prepare('UPDATE recurring_expenses SET is_active = 0 WHERE id = ?').run(recur.id);
      }
    });
  } catch (err) {
  ...
  try {
    ...
    debtors.forEach(debtor => {
      ...
      if (!existing) {
        insertNotification.run(debtor.user_id, message, 'payment_reminder', debtor.group_id);
      }
    });
  } catch (err) {
